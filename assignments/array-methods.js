...
let runnersLargeSizeShirt = runners.filter(function(item) {
  return item.shirt_size === "2XL" || item.shirt_size === "3XL" || item.shirt_size === "XL" || item.shirt_size === "L";
});
...
let donationAver = runners.reduce(function (acc, item) {
  return acc + item.donation;
}, 0);
console.log(ticketPriceTotal / runners.length);

// Problem 2
//Return email list
let emailList = [];
runners.forEach(function (items) {
  return emailList.push(items.email);
});
console.log(emailList);


// Problem 3
// Find items with XS size
let xs = runners.filter(function(item) {
